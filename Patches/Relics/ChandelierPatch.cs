...
namespace StatTheRelics.Patches.Relics {
    ...
    public static class ChandelierPatch {
        class ChandelierState {
            public int Energy { get; set; }
        }

        static void Prefix(Chandelier __instance, IReadOnlyList<Creature> participants, ref object __state) {
            try {
                ...
                __state = new ChandelierState {
                    Energy = Math.Max(0, ReflectionUtil.GetDynamicVarIntValue(__instance, "Energy"))
                };
            } catch { }
        }

        static void Postfix(Chandelier __instance, Task __result, object __state) {
            try {
                var state = __state as ChandelierState;
