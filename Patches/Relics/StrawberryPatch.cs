...
namespace StatTheRelics.Patches.Relics {
    ...
    public static class StrawberryPatch {
        ...
        static void Prefix(Strawberry __instance, ref object __state) {
            try {
                __state = new PickupState { BeforeMaxHp = GetMaxHp(__instance.Owner?.Creature) };
            } catch { }
        }

        static void Postfix(Strawberry __instance, Task __result, object __state) {
            try {
        ...
        static void Count(Strawberry relic, PickupState state) {
            try {
