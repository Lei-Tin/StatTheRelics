...
namespace StatTheRelics.Patches.Relics {
    ...
    public static class LeadPaperweightPatch {
        static readonly object Sync = new();
        static LeadPaperweight? activeRelic;

        class PickupState {
            public Dictionary<string, int> BeforeDeck { get; set; } = new(StringComparer.Ordinal);
        }

        static void Prefix(LeadPaperweight __instance, ref object __state) {
            try {
        ...
        static void Postfix(LeadPaperweight __instance, Task __result, object __state) {
            try {
        ...
        static void Clear(LeadPaperweight relic) {
            lock (Sync) {
                if (ReferenceEquals(activeRelic, relic)) activeRelic = null;
            }
        }

        internal static LeadPaperweight? ActiveRelic {
            get {
                lock (Sync) return activeRelic;
            }
        }

        internal static void SetOfferedCards(LeadPaperweight relic, IReadOnlyList<CardModel> cards) {
            try {
        ...
        static void Count(LeadPaperweight relic, PickupState state) {
            try {
    ...
    public static class LeadPaperweightChooseCardPatch {
        static void Prefix(IReadOnlyList<CardModel> cards, Player player) {
            try {
                var relic = LeadPaperweightPatch.ActiveRelic;
                if (relic == null || player == null || relic.Owner != player) return;
                LeadPaperweightPatch.SetOfferedCards(relic, cards);
            } catch { }
        }
    }
}
