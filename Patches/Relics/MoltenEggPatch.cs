...
namespace StatTheRelics.Patches.Relics {
    public static class MoltenEggPatch {
        class RewardState {
            ...
            public MoltenEgg? Relic { get; set; }
        ...
        internal static void CountCard(MoltenEgg relic, object card) {
            try {
        ...
        internal static object? CaptureRewardState(CardReward reward) {
            try {
                ...
                state.Relic = ReflectionUtil.FindRelic<MoltenEgg>(state.Owner);
                ...
                foreach (var creationResult in GetCreationResults(reward)) {
                    if (creationResult.ModifyingRelics?.OfType<MoltenEgg>().Any() != true) continue;
        ...
        internal static void CountRewardState(object? rawState) {
            try {
                var state = rawState as RewardState;
    ...
    public static class MoltenEggCardBeingAddedPatch {
        static void Postfix(MoltenEgg __instance, bool __result, CardModel newCard) {
            try {
                if (__result && newCard != null) MoltenEggPatch.CountCard(__instance, newCard);
            } catch { }
        }
    }
    ...
    public static class MoltenEggMerchantCardPurchasePatch {
        class PurchaseState {
            public MoltenEgg Relic { get; init; } = null!;
        ...
        static void Prefix(MerchantCardEntry __instance, ref object __state) {
            try {
                ...
                var relic = creationResult?.ModifyingRelics?.OfType<MoltenEgg>().FirstOrDefault();
                ...
                __result.ContinueWith(task => {
                    try {
                        if (task.Status == TaskStatus.RanToCompletion && task.Result.Item1) {
                            MoltenEggPatch.CountCard(state.Relic, state.Card);
                        }
                    } catch { }
                });
            } catch { }
    ...
    public static class MoltenEggCardRewardPatch {
        static void Prefix(CardReward __instance, ref object __state) {
            var state = MoltenEggPatch.CaptureRewardState(__instance);
        ...
        static void Postfix(Task<bool> __result, object __state) {
            try {
                ...
                if (__result == null) {
                    MoltenEggPatch.CountRewardState(__state);
                    return;
                }

                __result.ContinueWith(task => {
                    try {
                        if (task.Status == TaskStatus.RanToCompletion && task.Result) MoltenEggPatch.CountRewardState(__state);
                    } catch { }
                });
            } catch { }
        }
