...
namespace StatTheRelics.Patches.Relics {
    [HarmonyPatch(typeof(SandCastle), nameof(SandCastle.AfterObtained))]
    public static class SandCastlePatch {
        ...
        static void Prefix(SandCastle __instance, ref object __state) {
            try {
                var state = new State();
        ...
        static void Postfix(SandCastle __instance, Task __result, object __state) {
            try {
        ...
        static void Count(SandCastle relic, State state) {
            try {
                var upgraded = state.Cards
                    .Where(card => !card.WasUpgraded && IsUpgraded(card.Card))
                    .Select(card => DeckUtil.GetCardStorageValue(card.Card))
        ...
        static IEnumerable<CardState> CaptureCardStates(SandCastle relic) {
