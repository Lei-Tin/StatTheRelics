...
namespace StatTheRelics.Patches.Relics {
    public static class MysticLighterPatch {
        internal static int GetBonus(CardModel? cardSource, ValueProp props) {
            try {
                if (cardSource == null || !props.IsPoweredAttack() || cardSource.Enchantment == null) return 0;
                var relic = ReflectionUtil.FindRelic<MysticLighter>(cardSource.Owner);
                return relic == null ? 0 : Math.Max(0, ReflectionUtil.GetDynamicVarIntValue(relic, "Damage", 9));
            } catch {
                return 0;
            }
        }

        internal static MysticLighter? GetRelic(CardModel? cardSource) {
            try {
                return cardSource == null ? null : ReflectionUtil.FindRelic<MysticLighter>(cardSource.Owner);
            } catch {
                return null;
            }
        }

        internal static void CountResults(MysticLighter? relic, int bonusPerHit, Task<IEnumerable<DamageResult>> task) {
            try {
